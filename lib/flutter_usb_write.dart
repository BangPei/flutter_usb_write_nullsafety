...
class FlutterUsbWrite {
  ...
  factory FlutterUsbWrite() {
    if (_instance == null) {
      final methodChannel = const MethodChannel('flutter_usb_write/methods');
      final eventChannel = const EventChannel('flutter_usb_write/events');
  ...
  Future<bool?> write(Uint8List bytes) async {
    var args = <String, dynamic>{'bytes': bytes};
    try {
